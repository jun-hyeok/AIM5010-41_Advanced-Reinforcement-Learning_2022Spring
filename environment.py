...
class UnityToGymWrapper(gym.Env):
    ...
    def __init__(
        ...
        if self._allow_multiple_obs:
            ...
            self._observation_space = spaces.Dict(
                {"image": list_spaces[0], "vector": list_spaces[1]}
            )
        else:
    ...
    def reset(self) -> Union[List[np.ndarray], np.ndarray]:
        ...
        self.n_agents = len(decision_step)
        self.game_over = False
    ...
    def step(self, action: List[Any]) -> GymStepResult:
        ...
        action = np.array(action).reshape((self.n_agents, self.action_size))
    ...
    def _single_step(self, info: Union[DecisionSteps, TerminalSteps]) -> GymStepResult:
        if self._allow_multiple_obs:
            ...
            default_observation = {
                "image": default_observation[0],
                "vector": default_observation[1],
            }
        else:
        ...
        return (default_observation, float(info.reward[0]), done, {"step": info})
